...
// ======== Trinity Tabs (WG) ========
  function initTrinity() {
    const nodes = document.querySelectorAll('.trinity-node');
    const panels = document.querySelectorAll('.trinity-panel');
    ...
    function activate(targetId) {
      ...
      // Activate target
      const activeNode = document.querySelector(`.trinity-node[data-trinity-target="${targetId}"]`);
    ...
    nodes.forEach(node => {
      node.addEventListener('click', (e) => {
        const target = node.getAttribute('data-trinity-target');
